...
基于 STM32F103C8T6-MINI 板卡的上/下电时序控制器固件。监听 PA1 触发信号，
按可配置的延迟依次控制 PB1/PB10/PB12/PB14 四路电源使能输出，并通过串口
命令实现 LED 控制、IO 状态查询与时序逻辑查询。
...
| 引脚 | 方向 | 功能 | 说明 |
|------|------|------|------|
| PA1 | 输入 | 上电感知触发 | EXTI1 双边沿中断，内部下拉（防悬空误触发） |
...
| 中断 | 抢占 | 子 | 用途 |
|------|------|----|------|
| EXTI1 | 0 | 0 | PA1 边沿，启动时序（要求最低触发延迟） |
...
| PA1 边沿 | 动作 | 规则 |
|----------|------|------|
| 上升沿（低→高） | **正时序**：4 路依次**拉高** | 通道 i 在触发后 Ti 时刻拉高 |
| 下降沿（高→低） | **反时序**：4 路反序**拉低** | 通道 i 在触发后 (Tmax−Ti) 时刻拉低 |
...
| 触发输入 | PE9 / EXTI9 | PA1 / EXTI1（增加内部下拉） |
...
2. **上电初始状态**：MCU 复位时若 PA1 已为高电平，因无边沿不会自动补跑
   上电时序（与参考工程一致）；可用 `seq up` 手动触发。
